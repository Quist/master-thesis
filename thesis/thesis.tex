...
UHF, HF, tactical broadband and satellites\cite{ist-90}. % Mangler referanse på bruk av satelitter.
...
Three key issues that needs to be adressed in order to apply Web services in
tactical networks has been identified in previous work\cite{IST-118}:
...
communication. The dependency on end-to-end connection should be removed.
...
Web services generate overhead. Optimalization approuched should seek to reduce
the network traffic generated by Web services.
...
Military units that are participating in a tactical network are highly mobile
...
such as loss of signal or equipment malfunction. Durations of disconnections can
vary from seconds to days.

\paragraph{Limited}
The Date rate, how many bits that are sent per second, is limited in \gls{dil}
networks. Various aspects of the communcation link can attribute to this:
...
rates, more data to be transmitted again due to the data arring being incorrect.
This contributes to longer transmission time. In a military setting, an enemy
may delibarate sabotage the network with jamming, causing higher error rates.

\item[Link latency] The communication technology in use influces how fast data
...
Web Services developed for use in civilan networks directly into a \gls{dil}
...
the coming sections the different optimalization techniques are presented and a
overview is presented in \Cref{table:optimalization-overview}. Another issue
that needs to be addressed is, when we have identified optimalization
techniques, where do we place them? In the application itself or in a proxy?
...
require modification. This would require alot of resources and severly limit the
flexibility of using Web services. Another solution is, by using proxies, we can
place the optimalization there without altering the Web Services themselves. The
only thing required to do is to setup the application to send and receive data
through the proxy. The proxy will take care of the optimalization for tactical
networks. This seems like a more reasonable approach and is explored in this
thesis.
