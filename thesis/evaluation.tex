...
illustrated in \cref{figure-testing-environment-simple}.
...
\cref{appendix-netem-scripts} of the appendix lists the scripts used.
...
Cite at iPerf 3 har blitt evaluert til beste måleverktøy. Få med at det var det
beste til å måle der lave båndbredder er evaluert. Vurdere å ta med graf som viser de ulike
målingene med iPerf.
...
As performance indicators we use the average \gls{rtt} as perceived by the
client application and how the test utilizes the network. The latter is done by
capturing packets during a sample test run with Wireshark. It is worth noting
that this was only performed on one test case for each test, thus any variance
in use of the network may not have detected. This is especially true for the
networks with an chance of packet errors. However, it gives us an idea of the
network traffic during that test.
...
Another consistent trend was that the CoAP proxy struggled with the large
