...
\Cref{figure:kongsberg-results} shows the average RTT of the experiments
performed with tactical broadband. We notice the following:

\begin{itemize}

    \item For the average RTT we see the trends as in the software emulated
...
Hvordan løse det med CoAP. En konfigurasjon per proxy

\section{Summary}

In chapter we evaluted

Since the HTTP proxy works with messages of all sizes, we recommend using the
HTTP proxy. If it is known beforehand that messages will fit
...
\hline
  ...
  \gls{satcom} & HTTP proxy with GZIP & HTTP proxy with compression \\ \hline
