...
inter-proxy communication. We implement the proxy to support the Hypertext
Transfer Protocol (HTTP), the Advanced Message Queuing Protocol (AMQP) and the
Constrained Application Protocol (CoAP). Evaluations show that in most cases
using HTTP/TCP yielded the lowest Round-Trip Time (RTT). However, with small
message payloads and in networks with low data rates, CoAP had a lower RTT and
network footprint than HTTP/TCP.
