...
far most used Web service protocol. Furthermore, the third and forth premises
have some important important implications for our proxy. Our proxy must be able
...
the proxies are not required to be with HTTP, but rather a protocol that deals
...
intermittent} and \textit{limited}. Any optimization techniques should be able to
...
time. As we saw in the previous chapter, earlier work have identified the removal
...
\begin{description}
    ...
    \item[Redelivery Delay] The proxy should support retrying sending messages
    ...
    how many times a retransmission should attempted before giving up.
...
by handling it on the application layer.
...
in DIL networks in last chapter. In the following paragraphs we analyse them for
...
\begin{description}
    ...
    simply forwards HTTP-requests without using an alternative protocol.
    Although proxing Web service requests through proxies cause some overhead from
    ...
    Furthermore it is not TCP-friendly. For these reasons we conclude that UDP
    ...
    transmission. It support both the request-response and publish-subscribe message paradigms. We therefore recommend AMQP as a possible proxy pair communication method.

    \item[MQTT] MQTT is a publish-subscribe messaging protocol and is considered
    as lightweight and simple to implement. However, due to the inter-proxy
